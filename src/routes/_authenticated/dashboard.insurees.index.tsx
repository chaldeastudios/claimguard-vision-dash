      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            {isLoading ? "Loading…" : `${rows.length} insurees`}
          </p>
        </div>
            ...
            {rows.map((i) => (
              ...
              >
                <td className="px-5 py-3">
                  ...
                  >
                    {i.head && <Crown className="h-3.5 w-3.5 text-[color:var(--brand-orange)]" />}
                    {i.name}
                  </Link>
                </td>
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={6} className="px-5 py-10 text-center text-sm text-muted-foreground">
                  {isLoading ? "Loading insurees…" : "No insurees match the current search."}
                </td>
              </tr>
            )}
          </tbody>

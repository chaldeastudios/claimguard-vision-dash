...
function Settings() {
  ...
  // fullName starts null so the (async-loaded) profile value pre-fills the
  // input the first time, but local edits afterward always win.
  const nameValue = fullName ?? profile?.name ?? "";
  ...
  async function save() {
    ...
    try {
      const { error } = await supabase
        .from("profiles")
        .upsert(
          { id: profile.userId, full_name: nameValue, email: profile.email },
          { onConflict: "id" },
        );
  ...
  async function handleLogoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true);
    try {
      await uploadOrganizationLogo(profile.userId, file);
      await qc.invalidateQueries({ queryKey: CURRENT_PROFILE_QUERY_KEY });
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">Manage your reviewer profile and branding.</p>
      </div>
